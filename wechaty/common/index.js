 ...
 * @param {*} endWord 结尾备注
 */
async function getEveryDayRoomContent(sortId, endWord = '森小星') {
...
async function getEveryDayContent(date, city, endWord) {
  ...
  let str = `${today}<br>我们结婚的第${memorialDay}天<br><br>今天也是元气满满的一天,要开心噢^_^<br><br>今日天气<br>${
    weather.weatherTips
  }<br>${
  ...
  if (result.type == 'text') {
    ...
    if (room) {
      try {
        ...
        contact.say('森小星正在处理你的入群申请，请不要重复回复...');
        await lib.delay(10000);
    ...
    try{
      ...
      if(scheduleObj.isLoop){
        if(scheduleObj.time){
          ...
          if(res){
            await lib.delay(2000)
            contact.say('森小星已经把你的提醒牢记在小本本上了')
          }else{
            await lib.delay(2000)
